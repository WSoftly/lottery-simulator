...
function checkPrize(type, userMain, userExtra, drawMain, drawExtra) {
	...
	function checkDLTPrize(ticket, draw) {
		...
		if (matchedReds === 5 && matchedBlues === 2) return {
			level: "一等奖",
			prize: "浮动"
		};
		if (matchedReds === 5 && matchedBlues === 1) return {
			level: "二等奖",
			prize: "浮动"
		};
		if (matchedReds === 5 && matchedBlues === 0) return {
			level: "三等奖",
			prize: "10000元"
		};
		if (matchedReds === 4 && matchedBlues === 2) return {
			level: "四等奖",
			prize: "3000元"
		};
		if (matchedReds === 4 && matchedBlues === 1) return {
			level: "五等奖",
			prize: "300元"
		};
		if (matchedReds === 3 && matchedBlues === 2) return {
			level: "六等奖",
			prize: "200元"
		};
		if (matchedReds === 4 && matchedBlues === 0) return {
			level: "七等奖",
			prize: "100元"
		};
